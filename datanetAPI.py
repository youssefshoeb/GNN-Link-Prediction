...
'''
My Changes:
    -Files have been sorted for reproducible results
'''
    ...
    def get_port_stats(self):
        ...
        if (self.port_stats is None):
            raise DatanetException(
                "ERROR: The processed dataset doesn't have port performance data")
...
class Datanet21API:
    ...
    def __init__(self, data_folder, intensity_values=[], topology_sizes=[], shuffle=False):
        """
        ...
        for root, dirs, files in os.walk(self.data_folder):
            ...
            self._all_tuple_files.extend(
                [(root, f) for f in files if f.endswith("tar.gz")])
    ...
    def set_files_to_process(self, tuple_files_lst):
        ...
        for tuple in tuple_files_lst:
            if not type(tuple) is tuple or len(tuple) != 2:
                raise DatanetException(
                    "ERROR: The argument of set_files_to_process should be a list of tuples -> [(root_dir,file),...]")
            if (tuple not in self._all_tuple_files):
                raise DatanetException(
    ...
    def __iter__(self):
        ...
        for root, file in tuple_files:
            try:
                ...
                input_files = tar.extractfile(
                    dir_info.name + "/input_files.txt")
                if (dir_info.name + "/flowSimulationResults.txt" in tar.getnames()):
                    flowresults_file = tar.extractfile(
                        dir_info.name + "/flowSimulationResults.txt")
                else:
                    flowresults_file = None
                if (dir_info.name + "/linkUsage.txt" in tar.getnames()):
                    link_usage_file = tar.extractfile(
                        dir_info.name + "/linkUsage.txt")
                else:
                ...
                while(True):
                    ...
                    s._routing_file = os.path.join(
                        root, "routings", used_files[2])
                    ...
                    if (len(used_files) == 4):
                        self._graph_links_update(g, os.path.join(
                            root, "links_bw", used_files[3]))
                    ...
                    if (s._routing_file in self._routings_dic):
                        ...
                        routing_matrix = self._create_routing_matrix(
                            s._routing_file, len(g))
    ...
    def _process_flow_results(self, s):
        ...
        for i in range(0, len(r), int(math.sqrt(len(r)))):
            ...
            for j in range(i, i + int(math.sqrt(len(r)))):
                ...
                dict_result_agg = {'PktsDrop': aux_agg[2], "AvgDelay": aux_agg[3], "AvgLnDelay": aux_agg[4], "p10": aux_agg[5],
                                   "p20": aux_agg[6], "p50": aux_agg[7], "p80": aux_agg[8], "p90": aux_agg[9], "Jitter": aux_agg[10]}
                ...
                for flow in aux_result_flows:
                    ...
                    dict_result_tmp = {'PktsDrop': tmp_result_flow[2], "AvgDelay": tmp_result_flow[3], "AvgLnDelay": tmp_result_flow[4], "p10": tmp_result_flow[5],
                                       "p20": tmp_result_flow[6], "p50": tmp_result_flow[7], "p80": tmp_result_flow[8], "p90": tmp_result_flow[9], "Jitter": tmp_result_flow[10]}
                ...
                for flow in aux_traffic_flows:
                    ...
                    offset = self._timedistparams(
                        tmp_traffic_flow, dict_traffic)
                    if offset != -1:
                        self._sizedistparams(
                            tmp_traffic_flow, offset, dict_traffic)
                        # From kbps to bps
                        dict_traffic['AvgBw'] = q_values_for_flow[0] * 1000
                        dict_traffic['PktsGen'] = q_values_for_flow[1]
                        dict_traffic['TotalPktsGen'] = sim_time * \
                            dict_traffic['PktsGen']
...
#        User introduced single intensity
    ...
    def __iter__(self):
        ...
        for root, dirs, files in os.walk(self.data_folder):
            ...
            graphs_dic[root] = self._generate_graphs_dic(
            ...
            # Extend the list of files to process
            files.sort()
            tuple_files.extend([(root, f)
                               for f in files if f.endswith("tar.gz")])
        ...
        for root, file in tuple_files:
            ...
            if(feasibility_of_file != 0):
                try:
                    ...
                    results_file = tar.extractfile(
                        dir_info.name + "/simulationResults.txt")
                    traffic_file = tar.extractfile(
                        dir_info.name + "/traffic.txt")
                    status_file = tar.extractfile(
                        dir_info.name + "/stability.txt")
                    input_files = tar.extractfile(
                        dir_info.name + "/input_files.txt")
                    if (dir_info.name + "/flowSimulationResults.txt" in tar.getnames()):
                        flowresults_file = tar.extractfile(
                    ...
                    while(True):
                        ...
                        s._status_line = status_file.readline().decode()[:-1]
                        s._input_files_line = input_files.readline().decode()[
                            :-1]
                        ...
                        if (s._routing_file in routings_dic[root]):
                            ...
                            routing_matrix = self._create_routing_matrix(
                                g, os.path.join(root, "routings", s._routing_file))
                            routings_dic[root][s._routing_file] = routing_matrix

                        self._process_flow_results_traffic_line(
                            s._results_line, s._traffic_line, s._flowresults_line, s._status_line, s)
                        s._set_routing_matrix(routing_matrix)
                        s._set_topology_object(g)
                        it += 1
                        yield s
