...
class HetroGIN(torch.nn.Module):
    def __init__(self, input_channels: dict, embedding_size: int, num_layers: int = 2,
        ...
        self.convs.append(HeteroConv({
            ...
            ('link', 'connects', 'link'): GINLayer(input_channels['link'] + input_channels['link'], embedding_size, concat=True)}, aggr='sum'))
        ...
        for _ in range(num_layers - 1):
            self.convs.append(HeteroConv({
                ...
                ('link', 'connects', 'link'): GINLayer(embedding_size, embedding_size)}, aggr='sum'))
...
class FAST_ID_GINLayer(torch.nn.Module):
    ...
    and prelu activation.
    """
    def __init__(self, in_channels: int, out_channels: int, layer_num: int, concat: bool = False) -> None:
        super().__init__()

        # Mlp that maps node features x of shape [-1, in_channels] to shape [-1, out_channels] )
        self.mlp = torch.nn.Sequential(
            torch.nn.Linear(in_channels + 1, out_channels),  # add one for the cycle count
            torch.nn.BatchNorm1d(out_channels),
            torch.nn.ReLU(inplace=True),
            torch.nn.Linear(out_channels, out_channels)
        )
    ...
    def forward(self, x: Tensor, edge_index: Adj):
        # compute identity
        cycle_count = compute_identity(edge_index, self.layer_num + 1)
        x = torch.column_stack((x, cycle_count[:, self.layer_num]))
        x = self.conv(x, edge_index)
